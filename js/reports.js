...
const Reports = (() => {
  ...
  const renderDisclosure = ({ inputs, pvResult, fySummary, rouInitial, totalInterest, totalDep, totalPayments, amortRows }) => {
    ...
    // Reference date = end of first FY (balance sheet date), per Para 58(b)
    const reportingDate = Utils.fyRange(startDate, fyStartMonth).end;
    const maturityBands = buildMaturityBands(amortRows, fyStartMonth, reportingDate);
    container.insertAdjacentHTML('beforeend', `
      <div class="disclosure-section">
        <div class="disclosure-title">3. Maturity Analysis – Undiscounted Lease Payments (Para 58(b))</div>
        <div class="disclosure-text">
          <em style="color:var(--text-muted);font-size:12px;">Reference date: ${Utils.fmtDate(reportingDate)} (end of first financial year of lease)</em>
          <table class="maturity-table" style="margin-top:10px;">
            <thead><tr><th>Band</th><th>Undiscounted Payments (₹)</th></tr></thead>
            <tbody>
    ...
    container.insertAdjacentHTML('beforeend', `
      <div class="disclosure-section">
        <div class="disclosure-title">4. Key Assumptions &amp; Judgements (Para 52)</div>
        <div class="disclosure-text">
          <ul style="padding-left:20px;line-height:2;">
            ...
            <li><strong>Total Cash Outflow (Para 53(a)):</strong> ₹${Utils.fmtNum(totalPayments)}</li>
    ...
    const fyTbl = fySummary.map(r => `<tr>
      <td>${r.fy}</td>
    ...
    container.insertAdjacentHTML('beforeend', `
      <div class="disclosure-section">
        <div class="disclosure-title">5. Additional Mandatory Disclosures (Para 53–60)</div>
        <div class="disclosure-text">

          <strong>5(a) Financial Year-wise Lease Expense Summary (Para 58(a)):</strong>
          <table class="maturity-table" style="margin:10px 0 18px;">
            <thead><tr><th>Financial Year</th><th>Interest Expense (₹)</th><th>Depreciation (₹)</th><th>Total Cash Outflow (₹)</th></tr></thead>
            <tbody>${fyTbl}</tbody>
          </table>

          <strong>5(b) Short-term Lease Expense (Para 53(b)):</strong><br/>
          The Company does not have any leases with a lease term of 12 months or less that are accounted for under the short-term lease exemption. Accordingly, no short-term lease expense is recognised during the period. <em>(Nil)</em><br/><br/>

          <strong>5(c) Low-value Asset Lease Expense (Para 53(c)):</strong><br/>
          The Company does not have any leases of low-value assets that are accounted for under the low-value exemption. <em>(Nil)</em><br/><br/>

          <strong>5(d) Variable Lease Payments Not Included in Lease Liability (Para 53(d)):</strong><br/>
          There are no variable lease payments that do not depend on an index or rate and that are not included in the measurement of the lease liability. <em>(Nil)</em><br/><br/>

          <strong>5(e) Income from Sub-leasing (Para 53(e)):</strong><br/>
          The Company has not sub-leased any right-of-use assets during the period. <em>(Nil)</em><br/><br/>

          <strong>5(f) Future Cash Outflows Not Reflected in Lease Liability (Para 59):</strong><br/>
          The lease does not contain extension options, termination options, or residual value guarantees beyond those already included in the measurement of the lease liability. There are no potential cash outflows to which the lessee is exposed that are not already reflected in the lease liability recognised above.<br/><br/>

          <strong>5(g) Managing Liquidity Risk from Leases (Para 60):</strong><br/>
          The Company manages its liquidity risk arising from lease obligations by maintaining adequate cash reserves and committed credit facilities. The maturity profile of undiscounted lease obligations is disclosed in Section 3 above, which provides a basis for assessing near-term and long-term liquidity requirements from lease commitments.
        </div>
      </div>`);
  };

  // Build maturity bands from amort rows
  const buildMaturityBands = (amortRows, fyStartMonth, referenceDate) => {
    ...
    amortRows.forEach(row => {
      const monthsAway = Utils.monthsBetween(ref, row.date);
